// components/dashboard/AdvisorPerformanceSection.tsx
"use client";
...
function ClientPicker({
  ...
  return (
    <div className="space-y-8 pb-12">
      <div>
        <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#C49A3C] mb-1">Performance</p>
        <h1 className="text-3xl font-bold text-[#1c1917]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
          Select a Client
        </h1>
        <p className="text-sm text-[#78716c] mt-2">Choose the client whose portfolio you want to review.</p>
      </div>

      {clients.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <div className="w-16 h-16 rounded-2xl bg-[#1c1917] flex items-center justify-center mb-4">
            <TrendingUp className="w-7 h-7 text-[#C49A3C]" />
          </div>
          <h3 className="text-lg font-bold text-[#1c1917] mb-1" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
            No clients registered
          </h3>
          <p className="text-sm text-[#78716c]">Add a client from the Clients section to review their portfolio.</p>
        </div>
      ) : (
...
export function AdvisorPerformanceSection() {
